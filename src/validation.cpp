...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if(nHeight <= 10) // first 10 Blocks
    {
       nSubsidy = 50955000 * COIN; // 509,550,000 Premine
    }   
    else if(nHeight<=8809010) // next 8809000 Blocks
    {
        nSubsidy = 50 * COIN; // 440,450,000 Mintable
    }    
    else
    {
        nSubsidy = 0 * COIN;
...
{
    return g_blockman.m_stake_seen;
}
...
bool SignBlock(std::shared_ptr<CBlock> pblock, CWallet& wallet, const CAmount& nTotalFees, uint32_t nTime, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins)
{
    ...
    if (wallet.CreateCoinStake(pblock->nBits, nTotalFees, nTimeBlock, txCoinStake, key, setCoins))
    {
...
bool ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, BlockValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex, CBlockHeader *first_invalid,  const CBlockIndex** pindexFirst)
{
