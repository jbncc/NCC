...
public:
    ...
    CAmount GetStakeableCredit(bool fUseCache = true, const isminefilter& filter = ISMINE_SPENDABLE) const NO_THREAD_SAFETY_ANALYSIS;
...
private:
    ...
    void StakeNDCs(bool fStake, CConnman* connman, CTxMemPool* mempool);
...
public:
    ...
    bool CreateCoinStake(unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins);
