...
void CWallet::blockDisconnected(const CBlock& block, int height)
{
    ...
    for (const CTransactionRef& ptx : block.vtx) {
        ...
        CWalletTx::Confirmation confirm(CWalletTx::Status::UNCONFIRMED, /* block_height */ 0, {}, /* nIndex */ posInBlock);
        SyncTransaction(ptx, confirm);
    }
}
...
CAmount CWalletTx::GetStakeableCredit(bool fUseCache, const isminefilter& filter) const
{
    if (pwallet == nullptr)
        return 0;
...
CWallet::Balance CWallet::GetBalance(const int min_depth, bool avoid_reuse) const
{
    ...
    isminefilter reuse_filter = avoid_reuse ? ISMINE_NO : ISMINE_USED;
    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            const CAmount stakeableCredit{wtx.GetStakeableCredit(/* fUseCache */ false, ISMINE_SPENDABLE | reuse_filter)};
            if (tx_depth >= COINBASE_MATURITY) {
                ret.m_mine_stakeable += stakeableCredit;
            } else {
                ret.m_mine_immature_stakeable += stakeableCredit;
            }
...
CAmount CWallet::GetStake() const
{
    ...
    {
        const CWalletTx* pcoin = &(*it).second;
        if (pcoin->IsImmatureCoinStake() && pcoin->GetDepthInMainChain() > 0)
...
bool CWallet::CreateCoinStake(unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins)
{
...
int CWalletTx::GetBlocksToMaturity() const
{
    ...
    //NDCTODO Some wallets are still not abandoning coinstakes correctly
...
void CWallet::StakeNDCs(bool fStake, CConnman* connman, CTxMemPool* mempool)
{
    ::StakeNDCs(fStake, this, connman, mempool, stakeThread);
}

void CWallet::StartStake(CConnman *connman, CTxMemPool* mempool)
{
    m_enabled_staking = true;
    StakeNDCs(true, connman, mempool);
}
